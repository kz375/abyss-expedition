...
function playRigAction(name){
  ...
  const stageNode=rigModel.host.querySelector(".rig-root"),stageTrack=rigModel.locomotion.stage?.[name]||[],stageCurrent=getComputedStyle(stageNode).transform;stageNode.getAnimations().forEach(animation=>animation.cancel());const stageFrames=bodyPoses.map((pose,index)=>{const move=stageTrack[Math.min(index,stageTrack.length-1)]||{};return {offset:pose.at/action.duration,transform:`translate(${move.x||0}px,${move.y||0}px) rotate(${move.r||0}deg)`,easing:pose.easing||"ease-in-out"};}),stageSecond=stageFrames[1]?.offset||.25,stageBlend=Math.min(stageSecond*.35,(rigModel.actionSet.transitionMs||80)/action.duration),stageLead=name==="idle"?stageBlend:Math.min(stageSecond*.68,stageBlend+55/action.duration);stageNode.animate([{offset:0,transform:stageCurrent==="none"?stageFrames[0].transform:stageCurrent,easing:"ease-out"},{...stageFrames[0],offset:stageBlend},...(stageLead>stageBlend?[{...stageFrames[0],offset:stageLead}]:[]),...stageFrames.slice(1)],{duration:action.duration,iterations:action.loop?Infinity:1,easing:"linear",fill:"forwards"});
  const lowerBody=new Set(["thigh_L","shin_L","foot_L","thigh_R","shin_R","foot_R"]);
  rigModel.character.parts.forEach(id=>{const node=rigModel.host.querySelector(`[data-part="${id}"]`);if(!node)return;const current=getComputedStyle(node).transform;node.getAnimations().forEach(animation=>animation.cancel());const authored=frames.map(frame=>({offset:frame.offset,transform:poseTransform(id,frame),easing:frame.easing||"ease-in-out"})),second=authored[1]?.offset||.25,blendOffset=Math.min(second*.35,(rigModel.actionSet.transitionMs||80)/action.duration),bodyLead=name==="idle"||lowerBody.has(id)?blendOffset:Math.min(second*.72,blendOffset+70/action.duration),keyframes=[{offset:0,transform:current==="none"?authored[0].transform:current,easing:"ease-out"},{...authored[0],offset:blendOffset},...(bodyLead>blendOffset?[{...authored[0],offset:bodyLead}]:[]),...authored.slice(1)];node.animate(keyframes,{duration:action.duration,iterations:action.loop?Infinity:1,easing:"linear",fill:"forwards"});});
