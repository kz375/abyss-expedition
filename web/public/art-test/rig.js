...
async function loadRig(heroId="warrior"){
  ...
  const [skeleton,skin,baseActionSet,baseLocomotion,utility]=await Promise.all([fetch(rigAssetUrl(character.skeleton)).then(r=>{if(!r.ok)throw Error("Skeleton missing");return r.json();}),fetch(rigAssetUrl(character.skin)).then(r=>{if(!r.ok)throw Error("Skin missing");return r.json();}),fetch(rigAssetUrl(character.actions)).then(r=>{if(!r.ok)throw Error("Action set missing");return r.json();}),fetch(rigAssetUrl(character.locomotion)).then(r=>{if(!r.ok)throw Error("Locomotion set missing");return r.json();}),fetch(rigAssetUrl(character.utilityActions)).then(r=>{if(!r.ok)throw Error("Utility action set missing");return r.json();})]),actionSet={...baseActionSet,actions:{...baseActionSet.actions,...utility.actions}},locomotion={...baseLocomotion,profiles:{...baseLocomotion.profiles,...utility.profiles},stage:{...baseLocomotion.stage,...utility.stage}};
