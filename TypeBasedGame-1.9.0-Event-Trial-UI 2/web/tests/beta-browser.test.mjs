...
try{
  ...
  check(await evaluate('globalThis.ABYSS_BETA_BUILD==="1.9.0"'),'all scripts use UI Beta build 1.9.0');
  check(await evaluate('document.querySelectorAll("[data-hero]").length===5'),'five visible classes, Creator hidden');
  check(await evaluate('document.querySelectorAll(".hero-choice em").length===5&&$("loot-preview").innerText.includes("1–2")'),'class roles and difficulty loot preview are visible');
  ...
  await evaluate('game.enemy.telegraph="charge";game.enemy.next=game.clock+1000;render()');check(await evaluate('!$("danger-banner").hidden&&$("danger-banner").innerText.length>0'),'enemy wind-up displays a prominent danger banner');await evaluate('game.enemy.telegraph=null;render()');
