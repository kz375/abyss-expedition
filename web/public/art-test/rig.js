...
const RIG_DATA_VERSION=new URL(document.currentScript.src).searchParams.get("v")||"";
const RIG_CHARACTERS=globalThis.ART_TEST_ACTORS?.rigCharacters||Object.freeze({warrior:"warrior-v1",mage:"mage-v1",ranger:"ranger-v1",paladin:"paladin-v1",necromancer:"necromancer-v1",creator:"creator-v1"});
const rigAssetUrl=path=>{const url=new URL(path.replace(/^\/assets\/animation\//,""),RIG_ASSET_ROOT);if(RIG_DATA_VERSION)url.searchParams.set("v",RIG_DATA_VERSION);return url;};
...
async function loadRig(heroId="warrior"){
  ...
  const character=await fetch(rigAssetUrl(`characters/${characterId}.json`)).then(r=>{if(!r.ok)throw Error("Character rig missing");return r.json();});
  const [skeleton,skin,baseActionSet,baseLocomotion,utility,...packs]=await Promise.all([fetch(rigAssetUrl(character.skeleton)).then(r=>{if(!r.ok)throw Error("Skeleton missing");return r.json();}),fetch(rigAssetUrl(character.skin)).then(r=>{if(!r.ok)throw Error("Skin missing");return r.json();}),fetch(rigAssetUrl(character.actions)).then(r=>{if(!r.ok)throw Error("Action set missing");return r.json();}),fetch(rigAssetUrl(character.locomotion)).then(r=>{if(!r.ok)throw Error("Locomotion set missing");return r.json();}),fetch(rigAssetUrl(character.utilityActions)).then(r=>{if(!r.ok)throw Error("Utility action set missing");return r.json();}),...(character.actionPacks||[]).map(path=>fetch(rigAssetUrl(path)).then(r=>{if(!r.ok)throw Error(`Action pack missing: ${path}`);return r.json();}))]),actionSet={...baseActionSet,actions:Object.assign({},baseActionSet.actions,utility.actions,...packs.map(pack=>pack.actions||{}))},locomotion={...baseLocomotion,profiles:Object.assign({},baseLocomotion.profiles,utility.profiles,...packs.map(pack=>pack.profiles||{})),stage:Object.assign({},baseLocomotion.stage,utility.stage,...packs.map(pack=>pack.stage||{}))};
...
function worldPose(id,pose,cache=new Map()){
  ...
  const source=pose[id]||{},limit=rigModel.skeleton.limits?.[id],local=limit&&Number.isFinite(source.r)?{...source,r:Math.max(limit[0],Math.min(limit[1],source.r))}:source,joint=rigModel.joints.get(id)||{x:0,y:0},baseX=joint.x,baseY=joint.y;
  let value;
...
function modelPose(name,pose){
  ...
  if(hero==="creator"){adjust("torso",{r:3});adjust("head",{r:-3});if(name.startsWith("attack")||name==="break_strike"){adjust("upperArm_L",{r:-12});adjust("upperArm_R",{r:12});}}
  return out;
}
