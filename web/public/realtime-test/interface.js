...
function render(){if(!game)return;const m=game.enemy,active=!!game.heroId;
  ...
  $("save-status").textContent=saveError||(active&&!game.finished?tx("自动保存中 · 可导出存档跨设备恢复 · 种子不包含进度","Autosaving · Export for cross-device recovery · Seeds do not contain progress"):tx("正式版存档保持不变 · 实时测试版 1.7.0","Main-game saves stay untouched · Real-time Beta 1.7.0"));
...
function applyLocale(){document.documentElement.lang=profile.locale==="en"?"en":"zh-CN";document.title=tx("深渊远征 · 实时测试版","Abyss Expedition · Real-time Beta");
  ...
  $("title").textContent=tx("深渊流战 · 八层远征","Abyss Flow · Eight-Floor Expedition");$("eyebrow").textContent=tx("深渊远征 · 实时测试版 1.7.0","ABYSS EXPEDITION · REAL-TIME BETA 1.7.0");
...
function init(){
  if(globalThis.ABYSS_BETA_BUILD!=="1.7.0")throw new Error("Real-time Beta files are from different releases; deploy the complete 1.7.0 asset set");
