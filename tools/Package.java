...
class Package {
    public static void main(String[] args) throws Exception {
        Path desktop=Path.of("dist","desktop");
        if(!Files.isRegularFile(desktop.resolve("AbyssExpedition.jar"))) throw new IllegalStateException("Run java tools/Build.java first");
        ...
        Path releases=Path.of("releases","current");
        Files.createDirectories(releases);
        Path destination=Files.createTempDirectory(releases,"desktop-").toAbsolutePath();
        List<String> command=List.of(tool.toString(),"--type","app-image","--name","AbyssExpedition",
                "--app-version","1.0.0","--input",desktop.toAbsolutePath().toString(),
