...
try{
  ...
  check(await evaluate('globalThis.ABYSS_BETA_BUILD==="1.9.1"'),'all scripts use UI Beta build 1.9.1');
  ...
  await command('Page.navigate',{url:origin+'/art-test/'});await wait('document.querySelectorAll("[data-fx]").length===7');check(await evaluate('Object.keys(units).length===4&&document.querySelectorAll("#unit option").length===4'),'art lab exposes three regular enemies and one boss');await click('[data-fx="slash"]');check(await evaluate('!!document.querySelector(".slash")'),'art lab can preview attack effects');await click('#sequence');check(await evaluate('timers.length>=6'),'art lab can play a combined combat sequence');await evaluate('$("language").click()');check(await evaluate('document.documentElement.lang==="en"&&document.querySelector("h1").textContent.includes("Art")'),'art lab supports English UI');
