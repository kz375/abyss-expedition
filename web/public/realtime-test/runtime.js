...
function setGuard(active){if(!game||game.phase!=="battle"||game.paused||archiveKind||game.heroId!=="warrior")return false;const c=game.combat;if(active&&!c.guard){if(c.guardCooldownUntil>game.clock)return false;c.guard=true;c.guardStarted=game.clock;c.guardWardGranted=false;combatEvent("guard_start");globalThis.playCombatRig?.("guard");}else if(!active&&c.guard){const held=game.clock-c.guardStarted;c.guard=false;c.guardCooldownUntil=game.clock+CombatCore.GUARD_COOLDOWN_MS;if(held<CombatCore.HOLD_GUARD_MS){c.guardTapUntil=game.clock+CombatCore.PERFECT_GUARD_MS;combatEvent("guard_tap",{until:c.guardTapUntil});globalThis.playCombatRig?.("perfect_guard");}else globalThis.playCombatRig?.("idle");combatEvent("guard_end",{held,cooldown:CombatCore.GUARD_COOLDOWN_MS});}return true;}
