...
function setGuard(active){
  ...
  else if(!active&&c.guard){const held=game.clock-c.guardStarted;c.guard=false;c.guardCooldownUntil=game.clock+CombatCore.GUARD_COOLDOWN_MS;globalThis.playCombatGuard?.("end");if(held<CombatCore.HOLD_GUARD_MS){c.guardTapUntil=game.clock+CombatCore.PERFECT_GUARD_MS;combatEvent("guard_tap",{until:c.guardTapUntil});globalThis.playCombatRig?.("perfect_guard");}else globalThis.playCombatRig?.("idle");combatEvent("guard_end",{held,cooldown:CombatCore.GUARD_COOLDOWN_MS});}
  return true;
}
function damageHero(amount,reflect=true,continuous=false,attacker=null,kind="damage"){if(game.hp<=0)return 0;let guard=1;if(!continuous&&game.heroId==="warrior"){const c=game.combat,held=c.guard?game.clock-c.guardStarted:Infinity,perfect=c.guard&&held<=CombatCore.PERFECT_GUARD_MS||c.guardTapUntil>=game.clock;if(perfect){c.guardTapUntil=-1;combatEvent("perfect_guard",{target:attacker?.uid});globalThis.playCombatRig?.("perfect_guard");globalThis.playCombatGuard?.("perfect");if(typeof navigator!=="undefined"&&navigator.vibrate)navigator.vibrate([18,24,30]);if(attacker){addBreak(attacker,38,"perfect_guard");attacker.stunned=Math.max(attacker.stunned,.65);attacker.telegraph=null;attacker.next=Math.max(attacker.next,game.clock+650);}return 0;}if(c.guard){const reduction=CombatCore.guardReduction(held);guard=1-reduction;combatEvent("guard",{target:attacker?.uid,reduction});}}const e=effects(),incoming=(continuous?Math.max(0,amount):Math.max(1,amount-e.defense))*(game.statuses.sunder>0?1.25:1)*(1-e.damageReduction)*guard,absorbed=Math.min(game.shield,incoming);game.shield-=absorbed;if(absorbed>.5)feedback("player",absorbed,"shield",{vibrate:false});
...
function step(ms){if(!game||game.phase!=="battle"||game.paused||archiveKind||game.finished)return;
  ...
  if(game.combat.guard&&!game.combat.guardWardGranted&&game.clock-game.combat.guardStarted>=CombatCore.HOLD_GUARD_MS){game.combat.guardWardGranted=true;addWard(CombatCore.HOLD_WARD);combatEvent("guard_hold",{ward:CombatCore.HOLD_WARD});}
