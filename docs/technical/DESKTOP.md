...
- **独立桌面版**：运行 `abyss.desktop.DesktopLauncher`，或构建后运行 `dist/desktop/AbyssExpedition.jar`。使用独立窗口、输入框及确定按钮，不依赖终端中文字库和终端输入编码。菜单仍使用数字和回车，玩法不变。
...
第一条生成通用 `dist/desktop/AbyssExpedition.jar` 及三种启动脚本。运行该 JAR 的电脑需要 Java 17 或更新的桌面运行环境。
...
可通过 `java -Dabyss.saveDir="自选目录" -jar dist/desktop/AbyssExpedition.jar` 指定位置。桌面窗口启动时也显示实际目录。
