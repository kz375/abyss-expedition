...
function playCombatRig(name="idle"){
  ...
  const root=combatRig.host.querySelector(".combat-rig-root"),stage=combatRig.locomotion.stage?.[name]||[],current=getComputedStyle(root).transform,motionScale=name==="hit"?.18:.46,yawScale=name==="hit"?.2:.45;root.getAnimations().forEach(animation=>animation.cancel());root.animate([{offset:0,transform:current==="none"?"none":current},...authored.map((pose,index)=>{const move=stage[Math.min(index,stage.length-1)]||{};return {offset:pose.at/action.duration,transform:`translate(${(move.x||0)*motionScale}px,${(move.y||0)*motionScale}px) rotate(${(move.r||0)*(name==="hit"?.45:1)}deg) rotateY(${(move.yaw||0)*yawScale}deg)`,easing:pose.easing||"ease-in-out"};})],{duration,iterations:action.loop?Infinity:1,fill:"forwards"});
...
function combatEnemyEffect(type,index=0){
  ...
  const particles=index===2?26:index===1?22:18;for(let i=0;i<particles;i++){const spark=document.createElement("small");spark.className=i%4===0?"contact-shard":"contact-spark";spark.style.setProperty("--i",i);spark.style.setProperty("--a",`${i*(360/particles)}deg`);fx.append(spark);}if(index>0){const wave=document.createElement("u");wave.className="contact-ground-wave";fx.append(wave);}if(index===2){const execution=document.createElement("s");execution.className="contact-execution-line";fx.append(execution);}
