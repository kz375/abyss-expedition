...
function setGuard(active){if(!game||game.phase!=="battle"||game.paused||archiveKind||game.heroId!=="warrior")return false;const c=game.combat;if(active&&!c.guard){c.guard=true;c.guardStarted=game.clock;c.guardWardGranted=false;combatEvent("guard_start");}else if(!active&&c.guard){const held=game.clock-c.guardStarted;c.guard=false;if(held<CombatCore.HOLD_GUARD_MS){c.guardTapUntil=game.clock+CombatCore.PERFECT_GUARD_MS;combatEvent("guard_tap",{until:c.guardTapUntil});}combatEvent("guard_end",{held});}return true;}
