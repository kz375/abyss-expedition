...
public final class Language {
    ...
    private static final Map<String, String> ZH = Map.ofEntries(
        ...
        Map.entry("Discovery deck, balance pass, and safer boss encounters", "事件卡牌、平衡调整与更稳定的首领战更新"),
        Map.entry("Animated vitality, ward summons, and combat balance", "血条动画、护盾召唤物与战斗平衡更新"),
        ...
        Map.entry("Deal 240% attack damage and sunder enemy defense", "造成 240% 攻击伤害并削弱敌人防御"),
        Map.entry("Start every battle with 70 shield", "每场战斗开始时获得 70 护盾"),
        Map.entry("Deal 270% attack damage and set the enemy ablaze", "造成 270% 攻击伤害并点燃敌人"),
        Map.entry("Skills have 1 less cooldown turn", "技能冷却减少 1 回合"),
        Map.entry("Two arrows (140% each) that mark the target (+15% damage taken)", "射出两箭（每箭 140%），标记目标使其受到的伤害增加 15%"),
        Map.entry("15% chance for a small bonus hit after attacking", "普通攻击后有 15% 概率追加一次小额攻击"),
        Map.entry("Deal 215% damage, restore health, and Stun the enemy for 1 turn", "造成 215% 伤害、恢复生命并眩晕敌人 1 回合"),
        Map.entry("Gain 10 shield at the start of every turn", "每回合开始时获得 10 护盾"),
        Map.entry("Deal 200% damage, drain 50% of it as health, and inflict Poison", "造成 200% 伤害，吸取实际伤害的 50% 恢复生命，并施加中毒"),
        Map.entry("Enemies start battle Cursed: +25% damage taken for 3 turns", "战斗开始时诅咒敌人：受到伤害增加 25%，持续 3 回合"),
        Map.entry("Five strikes (100%-140%) and Empower yourself (+8% damage, 1 turn)", "连续攻击五次（100%～140%），并强化自身（伤害增加 8%，持续 1 回合）"),
        Map.entry("Instantly destroy enemies below 10% maximum health", "立即消灭生命低于最大生命 10% 的敌人"),
        ...
        Map.entry("A discovery was already chosen on this floor.", "本层已经选择过一张事件卡。"),
